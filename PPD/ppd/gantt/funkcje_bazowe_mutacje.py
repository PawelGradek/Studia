...
def TB_function_final(TASKS_BASE, edgeList):
        ...
        if len(progressList) > 0:
            ...
            for index in range(len(progressList) - 1, -1, -1):
                if progressList[index][1] == 0:
                    ...
                    print('--------------------------------------------------------------------------------',listTaskandTime)
                    lista_z_nr_zadan1.append(index)
                    print('lista z zadannnnnnnnnnnnniami1:', lista_z_nr_zadan1)
        ...
        if len(orderList) > 0:
            ...
            for listNumber in orderList:
                if TASKS_BASE[listNumber - 1][2] <= resources and check(listNumber, allowedEdges):
                    ...
                    lista_z_nr_zadan2.append(listNumber)
                    print('lista z zadannnnnnnnnnnnniami2:', lista_z_nr_zadan2)
    ...
    gnt.set_yticks([i for i in range(1, len(listazadan) + 1)])
    # # Labelling tickes of y-axis
    gnt.set_yticklabels([f'{i} ' for i in range(1, len(listazadan) + 1)])
    ...
    for i in range(len(listazadan)):
        ...
        gnt.text(x=(listazadan[i][1] - (listazadan[i][1] - listazadan[i][0]) // 2), y=(i + 0.15),  # x = (i*3)
                 s=f"{i + 1}", va='center', ha='center', color='black')
    # for i in listazadan[1::5]:
    #     # 1 od kiedy i długość 2 wysokosc-wspołrzędne i grubość
    #     gnt.broken_barh([(i[0], i[1]-i[0])], (1, 1), facecolors=('tab:blue'))
    # for i in listazadan[2::5]:
    #     # 1 od kiedy i długość 2 wysokosc-wspołrzędne i grubość
    #     gnt.broken_barh([(i[0], i[1]-i[0])], (2, 1), facecolors=('tab:red'))
    # for i in listazadan[3::5]:
    #     # 1 od kiedy i długość 2 wysokosc-wspołrzędne i grubość
    #     gnt.broken_barh([(i[0], i[1]-i[0])], (3, 1), facecolors=('tab:green'))
    # for i in listazadan[4::5]:
    #     # 1 od kiedy i długość 2 wysokosc-wspołrzędne i grubość
    #     gnt.broken_barh([(i[0], i[1]-i[0])], (4, 1), facecolors=('tab:pink'))

    plt.show()
    '''
    for k in range(len(listTaskandTime2)):# rozpoczęcie zadania
        dictTaskAndTime[k] = ['start', 'stop']
        dictTaskAndTime[k][0] = listTaskandTime2[k]

    for j in range(len(listTaskandTime)):# zakończenie zadania
        dictTaskAndTime[j][1] = listTaskandTime[j]
    print('słownik2', dictTaskAndTime)
    '''
    CT = currentTime + 1
    # print(f'Final orderList: {orderList}')
    return CT, listazadan
